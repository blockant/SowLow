...
class WinnerController{
    ...
    public static async getAllWins(req: Request, res: Response){
        try{
            const {userId, productId, page, limit, paginate}=req.query
            ...
            if(paginate==='true'){
                ...
                const foundWinners=await Winner.find(findQuery).populate('user', 'name email _id').populate('product', 'currency price name _id').populate('bid', 'bid_amount createdAt _id')
                return res.status(200).json({message: 'Success', winners: foundWinners})
            }
        }catch(err){
            Logger.error(err)
